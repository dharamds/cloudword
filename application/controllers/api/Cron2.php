...
class Cron2 extends CI_Controller {
    ...
    public function execute_background_requests($backup_id) {
        $getftp = $this->db->query("select bftp.*,fs.hostname,fs.password,fs.username,fs.port_no,fs.folder_path,fs.caption,fs.status as fstatus,p.folder_name from backupftp bftp INNER JOIN ftp_server fs on bftp.ftp_id =fs.ftp_id INNER JOIN project p ON bftp.project_id = p.project_id WHERE bftp.backup_id = " . $backup_id . "");
...
mirror --verbose --use-pget-n=8 -n --verbose / '.$baserootpath.'
EOF';
			
			$filePath = FCPATH.'cronjobs/'.$project_id.'.sh';
    ...
    public function folder_retrieve($backup_id){
            ...
            //$ff = fopen(APPPATH . "logs/" . $getftp->error_logfile, "a+");
				if($port == 22){
				$cmd  = 'lftp -u '.$username.','.$password.' -e "set ssl:verify-certificate no; set sftp:auto-confirm yes; du -hb; quit;" sftp://'.$hostname.':'.$port;
				}else{
					$cmd  = 'lftp -u '.$username.','.$password.' -e "set ssl:verify-certificate no; set sftp:auto-confirm yes; du -hb; quit;" ftp://'.$hostname.':'.$port;
				}
				$cmdData = exec($cmd);
    ...
    public function ftploopdataback($backup_id,$indexid,$callingdata=array(),$totalsize = 0,$projectdata = '', $connect = NULL) {
            ...
            if(empty($projectdata)){
                ...
                $projectdata = $this->db->query("select bftp.*,p.project_name,fs.hostname,fs.password,fs.username,fs.port_no,fs.folder_path,fs.caption,fs.status as fstatus,p.folder_name from backupftp bftp INNER JOIN ftp_server fs on bftp.ftp_id =fs.ftp_id INNER JOIN project p ON bftp.project_id = p.project_id WHERE bftp.backup_id = ".$backup_id."")->row();
                                        ...
                                        if ($fkey["type"] == 2) {
                                            ...
                                            array_push($folderdata, array("localrootfolder" => $lf, "remoterootfolder" => $rf));										
												...
												if ($ftpcalling->download($key["remoterootfolder"] . $fkey["filename"], $key["localrootfolder"] . $fkey["filename"])) {
													...
													$msg = '<p> download file please check specified path : ' . $key["remoterootfolder"] . $fkey["filename"].'</p>';
														fwrite($ff, $msg);
											...
											/*
											if(!is_file($key["localrootfolder"] . $fkey["filename"])){
												//echo $key["remoterootfolder"] . $fkey["filename"]; 												
                                                if ($ftpcalling->download($key["remoterootfolder"] . $fkey["filename"], $key["localrootfolder"] . $fkey["filename"])) {
                                                    $totalsize+= $fkey["size"];
                    ...
                    if ($db->status == "failed" || $db->status == "processing") {
					   ...
					   if ($returndata["status"] == "success") {
                            ...
                            if(sizeof($getproject_setting) > 0){
								 ...
								 $this->db->where("backup_id", $backup_id);
								 $this->db->update("backupsql", ["foldersdata" => json_encode($folderdata), "completed_database" => $cmpdb,"status" =>"processing"]);
							}
                        } else {
...
exit;
    ...
    public function execute_restore_ftp_process($restore_id){
		...
		$bkpdata = $this->db->query("select bftp.*,fs.hostname,fs.password,fs.username,fs.port_no,fs.folder_path,fs.caption,fs.status as fstatus,p.folder_name,p.project_name from backupftp bftp INNER JOIN ftp_server fs on bftp.ftp_id =fs.ftp_id INNER JOIN project p ON bftp.project_id = p.project_id WHERE bftp.backup_id = ".$restoredata->backup_id." ")->row();
		...
		// calculating download file size
				...
				if($total_size !== (int)$bkpdata->total_remote_size){
...
set ssl:verify-certificate no
set sftp:auto-confirm yes
mirror -R -c --Remove-source-files --Remove-source-dirs '.$filepath.' '.$bkpdata->remoteroot_folder.'
EOF';
			
			$filePath = FCPATH.'cronjobs/'.$bkpdata->project_id.'_r_.sh';
			if (write_file($filePath, $filetext, 'w'))
				{
						//$this->db->where("backup_id",$backup_id)->update("backupftp",["cron_status" => 1]);
						$cmd = 'bash '.$filePath;
