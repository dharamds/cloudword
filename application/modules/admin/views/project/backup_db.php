    ...
    <div class="row mr-0">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="<?= base_url('admin/project/manage_backup/'.base64_encode($db_server_details->project_id))?>"><?=$this->lang->line("backups") ?></a></li>
      ...
      $('[data-toggle="tooltip"]').tooltip({container: 'body'})
          var dttt =  $('#backuplist').DataTable({
              ...
              'processing': true,
                ...
                'serverMethod': 'post',
                'ajax': {
                   'url':'<?=base_url()?>admin/project/getdb_ajax',
                   'data': function(data){
   ...
   function viewlogs(backup_id){
    if($("#bkpmodal_id").val() == ""){
        $("#bkpmodal_id").val(backup_id);
                     ...
                     $.ajax({
                            url:"<?php echo base_url();?>admin/project/backupdblogs",
                            type:"post",
                            beforeSend:function(){
                               $("#cover-spin").show()
                            },
