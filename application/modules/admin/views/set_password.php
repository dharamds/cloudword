    ...
    $("#resetpassword_form").validate({
			rules: {
				'username': {
					required: true,
					remote   : {
                        url:"<?php echo base_url();?>admin/login/getCheckUserNameExistForSetPass",
					 	type: 'POST',
        ...
        var submitForm = function (){ 
            $('.message').html('');
            $.ajax({
                url:"<?php echo base_url();?>admin/login/get_set_password",
                method: "POST",
                ...
                success: function (result) {
                    if(result.code == 200)
                    {
                        ...
                        setTimeout(function(){ window.location.href = "<?php echo base_url();?>admin/login" }, 5000);
                    }else{
                        //$(".loader").hide();
