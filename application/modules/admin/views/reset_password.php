    ...
    $("#resetpassword_form").validate({
			rules: {
				'username': {
					required: true,
					remote   : {
                        url:"<?php echo base_url();?>admin/login/getCheckUserNameExist",
					 	type: 'POST',
        ...
        var submitForm = function (){ 
            $('.message').html('');
            $.ajax({
                url:"<?php echo base_url();?>admin/login/get_change_password",
                method: "POST",
                ...
                success: function (result) {
                    if(result.code == 200)
                    {
                        ...
                        setTimeout(function(){ window.location.href = "<?php echo base_url();?>admin/login" }, 4000);
                    }else{
                        //$(".loader").hide();
