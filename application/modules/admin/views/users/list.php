			...
			<div class="col-md-12">
				<div class="panel panel-default panel-grid">
					...
					<div class="panel-body no-padding p-0">
						<!-- Custom Filter -->
						<div>
						     <select id='filterRole'  class="form-control" style="width: 25%;">
						       <option value=''> Select User Role </option>
						       <?php foreach ($roles as $role){ ?> 
						       		<option value='<?= $role->role_id ?>' <?php if($selected_role == $role->role_id) echo "Selected"; ?>><?= $role->role_name ?></option>
						       <?php } ?>
						     </select>
						</div>
						<br>
...
<script type="text/javascript">
	...
	function deleteUser(client_id){
	 ...
	 //        e.preventDefault();
	 //    	$.ajax({
	 //           url:"<?php //echo base_url();?>admin/users/save/edit",
	 //           type:"post",
	 //           beforeSend:function(){
	 //           		$("#cover-spin").show()
	 //           },
