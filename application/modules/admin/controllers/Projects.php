...
class Projects extends MX_Controller {
    ...
    function save() {
        ...
        if (count($error_data) > 0) {
            ...
            $data = array("project_name" => $this->encryption->encrypt($name), "slug" => $slug, "added_date" => $date, "folder_name" => $this->encryption->encrypt($folder_name), "datetimestamp" => $added_date, "client_id" => $user_id, "url" => $this->encryption->encrypt($url));
    ...
    public function set_ftp() {
        ...
        if ($ftpstatus == 1) {
            ...
            $data = array("protocol_type" => $this->encryption->encrypt($protocol_type), "hostname" => $this->encryption->encrypt($hostname), "username" => $this->encryption->encrypt($username),
            //"password"      => $this->encryption->encrypt($password),
            "port_no" => $this->encryption->encrypt($port_no), "url" => $this->encryption->encrypt($domain_url), "root_path" => $this->encryption->encrypt($root_path));
    ...
    public function set_mysql() {
        ...
        if ($sqlcheck == 1) {
            ...
            $data = array("mdatabase_name" => $this->encryption->encrypt($mdatabase_name), "mhostname" => $this->encryption->encrypt($mhostname), "musername" => $this->encryption->encrypt($musername), "mpassword" => $this->encryption->encrypt($mpassword));
    ...
    public function connecttodbandgettablecount($host, $user, $pass, $dbname, $tables = '*', $path, $project_id, $user_id, $file_name) {
        ...
        try {
            ...
            if (mysqli_connect_errno()) {
                ...
                return array("status" => "success", "msg" => "Table fetched Successfully", "path" => $path, "file_name" => $file_name, "tables" => $tables, "tablescount" => $tablescount);
            }
        }
        catch(\Throwable $e) {
    ...
    public function putreqcron() {
        ...
        if ($ftpcalling->connect($config)) {
            ...
            if (count($fdatadata) > 0) {
                ...
                $ftpbkp_id = $this->db->insert_id();
                echo json_encode(array("status" => "success", "msg" => "Request to take backup submitted successfully please check latest FTP backup status on URL : <a href='" . base_url() . "admin/backup'>Backup List</a>"));
            } else {
