    ...
    <div class="row mr-0">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="<?= base_url('client/project/manage_backup/'.base64_encode($backup_data->project_id))?>"><?=$this->lang->line("backups") ?></a></li>
          <li class="breadcrumb-item"><a href="<?= base_url('client/project/backup_db/'.$backup_data->db_id)?>"><?=$this->encryption->decrypt($server_data->caption) ?></a></li>
        ...
        $('[data-toggle="tooltip"]').tooltip({container: 'body'})
          var dttt =  $('#backuplist').DataTable({
              ...
              'processing': true,
                ...
                'serverMethod': 'post',
                'ajax': {
                   'url':'<?=base_url()?>client/backup/restore_process_db_ajax',
                   'data': function(data){
  ...
  function viewlogs(restore_id){
    ...
    if($("#ftprestore_id").val() == ""){
        $("#ftprestore_id").val(restore_id);
                     ...
                     $.ajax({
                            url:"<?php echo base_url();?>client/backup/restorelogsdb",
                            type:"post",
                            beforeSend:function(){
                               $("#cover-spin").show()
                            },
