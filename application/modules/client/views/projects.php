										...
										?>
											...
											<td>
												...
												<?php if($proj->ftp_status == 1){
													...
													<a href="javascript:" onclick='backupftpbackground(<?php echo $proj->project_id;?>)' class="btn btn-danger">Backup Project in Background</a>

												<!-- <a href="<?php //echo base_url(); ?>client/projects/listftp/<?php //echo base64_encode($proj->project_id);?>" class="btn btn-danger"><?php //s $this->lang->line("bkp_ftp")?></a> -->
												<?php  } ?>


												<?php if($proj->sql_status){ ?>												
												<a href="javascript:" onclick="sqlbkp(<?=$proj->project_id;?>)" class="btn btn-danger "><?= $this->lang->line("bkp_sql")?></a>
												<?php  } ?>


												<a href="<?= base_url()?>client/optimizations/alive_system/<?= base64_encode($proj->project_id)?>"  class="btn btn-info"><?= $this->lang->line("chk_sys")?></a>
...
<div class="modal fade prolivestatus" id="prolivestatus-Modal" tabindex="-1" role="dialog">
	<div class="modal-dialog modal-lg" role="document" style="width: 500px;">
		<div class="modal-content">
			...
			<div class="modal-body">
				<strong style="color: green" id="backlivestatus"></strong>
			</div>
		</div>
	</div>
</div>
...
function addport(vv){
	...
	function startbkp(proid){
		...
		$.ajax({
			url:"<?php echo base_url();?>client/projects/startbkp",
			method:"post",
			data:{proid:proid},
			dataType:'json',
					...
					if(data.tablescount > 0){
					    ...
					    $.each(data.tables, function (tableKey, tableName) {
					    	...
					    	setTimeout(function(){ 
						    	...
						    	$.ajax({
									url:"<?php echo base_url();?>client/projects/backuptableonebyone",
									method:"post",
...
<script type="text/javascript">
					function backupftp(project_id){
						...
						$.ajax({
				           url:"<?php echo base_url();?>client/projects/backupftp",
				           type:"post",
				            ...
				            success:function(dat){
				            	...
				            	for(var j=0;j<dd.length;j++){
				            		...
				            		if(temp == "." || temp ==".." || temp =="/.." || temp =="/."){
				            			...
				            			$.ajax({
								           url:"<?php echo base_url();?>client/projects/ftploopdata",
								           type:"post",
								           timeout:72000000,
								            ...
								            success:function(resdata){
								            	...
								            	if(resdata.status== "success"){
								            		...
								            		if(perc == 100){
								            			...
								            			$("#successmsggg").show();
								            // 			$.ajax({
												        //    url:"<?php //echo base_url();?>client/projects/zipproject",
												        //    type:"post",
						...
						function readlogfile(error_logfile){
							   						$.ajax({
												           url:"<?php echo base_url();?>client/projects/logfile",
												           type:"post",
						...
						function backupftpbackground(project_id){
							...
							if(r){
								$.ajax({
							           url:"<?php echo base_url();?>client/projects/putreqcron",
							           type:"post",
							            data: {project_id:project_id},
							            dataType: 'json',
							            success:function(dat){
