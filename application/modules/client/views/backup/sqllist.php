							...
							<tbody>
								...
								foreach($backups as $bkp) {
									...
									<tr>
										...
										<td>
										<?php	
										$currentFile = "./projects/".$bkp->folder_name."/mysql_server/".$bkp->file_name;
										...
										<a style="margin:5px;min-width: 40px;" href="javascript:" data-toggle="tooltip" data-placement="top" title='<?= $this->lang->line("restore_bkp")?>' class="btn btn-primary waves-effect waves-light" onclick='restorebkp(<?=$bkp->backup_id;?>)'><i class="flaticon-backup-2"></i></a>
										...
										<a style="margin:5px;min-width: 40px;" data-toggle="tooltip" data-placement="top" title='<?= $this->lang->line("download")?>' href="<?=base_url()?>client/backup/downloadsql/<?=$bkp->backup_id;?>" class="btn btn-primary waves-effect waves-light"> <?= $this->lang->line("download")?></a>
...
function startbkprestore(bkp_id){
	...
	$.ajax({
		url:"<?php echo base_url();?>client/backup/startrestore",
		method:"post",
		data:{bkp_id:bkp_id},
		dataType:'json',
				...
				if(data.querycount > 0){
				    ...
				    $.each(data.tabledata, function (tableKey, tableQuery) {
				    	...
				    	setTimeout(function(){ 
					    	...
					    	$.ajax({
								url:"<?php echo base_url();?>client/backup/restoretableonebyone",
								method:"post",
								data:{tableQuery:tableQuery,tKey:tKey,bkp_id:bkp_id},
								dataType:'json',
								async: false,
	...
	function deletebkp(bkp_id){
		...
		if($chk){
			$.ajax({
				url:"<?php echo base_url();?>client/backup/deletesql",
				method:"post",
				 beforeSend:function(){
				           		$("#cover-spin").show()
				           },
