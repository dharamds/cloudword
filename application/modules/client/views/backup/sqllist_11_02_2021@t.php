							...
							<tbody>
								...
								foreach($backups as $bkp) {
									...
									<tr>
										...
										<td><a style="margin:5px;" href="javascript:" class="btn btn-primary btn-round waves-effect waves-light" onclick='restorebkp(<?=$bkp->backup_id;?>)'><i class="flaticon-sand-clock-1"></i> <?= $this->lang->line("restore_bkp")?></a>
										...
										<a style="margin:5px;" href="<?=base_url()?>client/backup/downloadsql/<?=$bkp->backup_id;?>" class="btn btn-primary waves-effect waves-light"> </i><?= $this->lang->line("download")?> </a>
...
<script type="text/javascript">
	...
	function restorebkp(bkp_id){
		...
		if($chk){
			$.ajax({
				url:"<?php echo base_url();?>client/backup/sqlrestore",
				method:"post",
				 beforeSend:function(){
				           		$("#cover-spin").show()
				           },
