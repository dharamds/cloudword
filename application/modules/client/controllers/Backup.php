...
class Backup extends MX_Controller 
{
    ...
    public function index(){
        ...
        $this->load->view("client/backup/list",$data);
    }
    ...
    public function sqlbkp(){
       ...
       if(!empty($data["backups"])){
        ...
        $this->load->view("client/backup/sqllist",$data);
        ...
        $bkpdata = $this->db->query("select bdbs.*,ms.mhostname,ms.mpassword,ms.musername,ms.port_no,ms.folder_path,ms.status as dbstatus,p.folder_name,ms.caption from backupsql bdbs INNER JOIN mysql_server ms on bdbs.db_id =ms.mysql_id INNER JOIN project p ON bdbs.project_id = p.project_id WHERE bdbs.backup_id = " . $bkp_id . "")->row();
         ...
         set_error_handler(function ($err_severity, $err_msg, $err_file, $err_line, array $err_context) {
            ...
            try {
                ...
                $dbname = $db_name;
                $restorefile = "./projects/".$this->encryption->decrypt($bkpdata->folder_name)."/db_server/".$bkpdata->folder_path."/".$file_name;
    ...
    public function downloadftp($id = NULL) { 
        ...
        if($id){
            ...
            if(!empty($bkpdata)){
                ...
                redirect(base_url()."client/project/backup_ftp/");
                exit;    
            }
    ...
    public function restore_db($backup_id = NULL){
            ...
            $data["page"] =   "projects";
            $this->load->view("client/project/restoredb",$data);
    }
    public function restore_process($ftp_id){
            $data["ftp_data"] = $this->db->query("select fs.* from ftp_server fs inner join project p on p.project_id = fs.project_id where fs.ftp_id = ".$ftp_id." order by fs.ftp_id DESC")->row();
            ...
            $data["page"] =   "projects";
            $this->load->view("client/project/restore_process",$data);
    }
    ...
    public function restore_process_db($backup_id){
            ...
            $data["backup_id"] =   $backup_id;
            $data["page"] =   "projects";
            $this->load->view("client/project/restore_process_db",$data);
        }else{
            redirect(base_url()."client/backup/restore_db/".$backup_id);
        }
    }
