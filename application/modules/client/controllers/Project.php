...
class Project extends MX_Controller {
    ...
    public function index(){
        ...
        $prodata = $this->db->query("select p.* from project p where p.client_id = ".$user_id." order by p.project_id DESC");
        $data["notification_setup"] = $this->db->query("select * from project_setting where client_id = ".$user_id." ")->row();
        $data["project_count"] = $prodata->num_rows();
        $data["page"] = "projects";
        $data["projects"] = $this->get_decrypt_project_object($prodata->result());
        $this->load->view("client/project/project", $data);
    }
   public function get_decrypt_project_object($data) {
    ...
    public function manage_backup($project_id = NULL){
        ...
        $project_id = base64_decode($project_id);
	        ...
	        $data["page"] = "projects";
	        $this->load->view("client/project/manage_backup", $data);
    }
    public function add_ftp($project_id=NULL){
    	if($project_id != NULL){
	    	...
	    	$data["fun"] = "ftp";
	    	$data["page"] = "projects";
		    $this->load->view("client/project/add_ftp_db", $data);
	    }
    }
    ...
    public function backup_ftp($ftp_id=NULL){
            if($ftp_id != NULL){
                ...
                $data["page"] = "projects";
                $this->load->view("client/project/backup_ftp", $data);   
            }
    }
        ...
        $totalRecordwithFilter = $totalRecords;
         ...
         foreach($records as $record ){
                                   $action =     '<a  href="javascript:" class="btn btn-primary" onclick="viewlogs('.$record->backup_id.')">'.$this->lang->line("view_logs").'</a>';
                                       ...
                                       $action .= '<a  href="'.base_url().'client/backup/downloadftp/'.$record->backup_id.'" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="'.$this->lang->line("download").'">'.$this->lang->line("download").'</a><a style="margin:5px;min-width: 40px;" href="javascript:" class="btn btn-primary waves-effect waves-light" onclick="restorebkptest('.$record->backup_id.')" data-toggle="tooltip" data-placement="top" title="'.$this->lang->line("restore_bkp").'">'.$this->lang->line("restore_bkp").'</a><a style="margin:5px;min-width: 40px;" href="'.base_url().'client/project/restore_to_server/'.$record->backup_id.'" class="btn btn-primary waves-effect waves-light"  data-toggle="tooltip" data-placement="top" title="'.$this->lang->line("restore_bkp_to_other").'"> '.$this->lang->line("restore_bkp_to_other").'</a>';
     ...
     public function getdb_ajax(){
        ...
        $totalRecordwithFilter = $totalRecords;
         ...
         foreach($records as $record ){
                                   $action ='<a  href="javascript:" class="btn btn-primary" onclick="viewlogs('.$record->backup_id.')">'.$this->lang->line("view_logs").'</a>';
                                  if($record->status == "success"){
                                    $action .= '<a href="'.base_url().'client/backup/restore_db/'.$record->backup_id.'" class="btn btn btn-primary">'.$this->lang->line("download").'</a><a href="'.base_url().'client/backup/restore_db/'.$record->backup_id.'" class="btn btn btn-info">'.$this->lang->line("restore_bkp").'</a>';
                                  }
            $st =  $record->status == 'processing' ? 'warning' : 'success';
            $status = '<span class="badge badge-'.$st.'">'.$this->lang->line($record->status).'</span>';
            $data[] = array( 
    ...
    public function backup_db($db_id = NULL){
            if($db_id != NULL){
                ...
                $data["db_backup_list"] = $dbdata;
                 $data["db_id"] = $db_id;
                $data["page"] = "projects";
                $this->load->view("client/project/backup_db", $data);   
            }
    }
    public function putdbcron(){
		...
		$getftpStorage = $this->db->get_where("client_storage",array("user_id" => $db_data->client_id))->row();
		if((int)$getftpStorage->used_db_storage >= $getftpStorage->db_storage){
                echo json_encode(array("status" => "failed", "msg" => $this->lang->line("db_storage_quota_exceed")));
                exit;
		} 
    ...
    public function putftpcron(){
		...
		//check available storage
		$getftpStorage = $this->db->get_where("client_storage",array("user_id" => $getftp->client_id))->row();
		if((int)$getftpStorage->used_ftp_storage >= $getftpStorage->ftp_storage){
                echo json_encode(array("status" => "failed", "msg" => $this->lang->line("ftp_storage_quota_exceed")));
                exit;
		}
    ...
    public function edit_ftp($ftp_id = null)
    {
        ...
        $data["page"] = "edit_ftp";
        $this->load->view("client/project/edit_ftp", $data);
    }
    ...
    public function edit_db($db_id = null)
    {
        ...
        $data["page"] = "edit_db";
        $this->load->view("client/project/edit_db", $data);
    }
     ...
     public function putftprestorecron(){
        ...
        try {
                ...
                public function set_notification(){
                        ...
                        $send_to_mails = $this->input->post("send_to_mails");
                        $notify_id = $this->input->post("notify_id");
                        if($setting_id !=""){
                            $this->db->where("setting_id",$setting_id);
                            $chk = $this->db->update("project_setting",["notify_id" => $notify_id,"send_to_mails" => $send_to_mails]);
                        }else{
                            $chk = $this->db->insert("project_setting",["client_id" => $user_id,"notify_id" => $notify_id,"send_to_mails" => $send_to_mails]);
                        }
                        if($chk){
                             $output = array("status" => "success", "msg" => $this->lang->line("notification_up_msg"));
                        }else{
                             $output = array("status" => "failed", "msg" => $this->lang->line("something_wrong"));
                        }
                        echo json_encode($output);
                ...
                public function restore_to_server($backup_id=NULL){
                    ...
                    $data["page"] = "project";
                    $this->load->view("client/project/restore_to_remote_process", $data);
                }
                public function fetch_remote_server_data(){
                    ...
                    $this->load->library('upload', $config);
                                        ...
                                        $password = !empty(trim($this->input->post("password"))) ? trim($this->input->post("password")) :'testing';
            ...
            if ( ! $this->upload->do_upload('rsa_file')) {
                ...
                if($key_filepath){
                    include(APPPATH.'third_party/phpseclib/Crypt/RSA.php');
                             ...
                             $rsa = new Crypt_RSA();
                            $rsa->loadKey(file_get_contents($key_filepath));
                            $callingftp = new Net_SSH2($hostname);
                            if (!$callingftp->login($username, $password, $rsa)) {
                                $ftpstatus = 0;
                            }else{
                                $ftpstatus = 1;  
                            }
                                        ...
                                        if($ftpstatus == 1 && count($error_data) == 0){
                                            ...
                                            if($key_filepath){
                                                $fdatadata[] = $callingftp->exec('ls');

                                                foreach ($fdatadata as $fkey){
                                            
                                                            array_push($folderdata, ["count" => $cnt,"folder_name" =>"/".$fkey."/"]);
                                                            $cnt++;
                                            ...
                                            $output = array("status" => "success","folderdata" => $folderdata, 'public_key' => $key_filepath);
                                        }else{
                                    ...
                                    public function putftprestorecron_server(){
                                        ...
                                        $remote_path = ($this->input->post("custom_remote_path")) ? $this->input->post("custom_remote_path") : $this->input->post("remote_path");
                                        $key_filepath = $this->input->post("public_key");
