var AddQuestionSectionPart = function () {
    var webroot,quizID,selectedTable;

    var get_open_questions_popup = function(){
        $.ajax({
            url      :   webroot+"assessments/get_active_questions_no",
            method   :   "POST",
            data     : {'quizID':quizID},
            dataType : "html",        
          ...
          selectedTable = $('#selected_quiz_table').DataTable({
            ...
            "ajax": {
                ...
                "data": function (data) {
                    ...
                    /*data.system_category = $('#selected_filter_search #selected_system_category option:selected').val();
                    data.system_tag = $('#selected_filter_search #selected_system_tag option:selected').val();
                    data.assessment_category = $('#selected_filter_search #selected_assessment_category option:selected').val();
                    data.assessment_tag = $('#selected_filter_search #selected_assessment_tag option:selected').val();
                    data.course_tag = $('#selected_filter_search #selected_course_tag option:selected').val();
                    data.module_tag = $('#selected_filter_search #selected_module_tag option:selected').val();
                    data.lesson_tag = $('#selected_filter_search #selected_lesson_tag option:selected').val();
                    data.outcomes_category = $('#selected_filter_search #selected_learning_outcomes option:selected').val();
                    data.standard_category = $('#selected_filter_search #selected_learning_standards option:selected').val();
                    data.compentencies_category = $('#selected_filter_search #selected_learning_competencies option:selected').val();
                    data.skills_category = $('#selected_filter_search #selected_learning_skills option:selected').val();*/
                },
            },
    ...
    var getRemoveQuestionFromSections = function(questions, quizID){

        $.ajax({
            url: webroot+"assessments/questions/selectedRemove",
            method: "POST",
            data:{'selected_record':questions,'quizID':quizID},
            dataType:'json',
            beforeSend: function(){
    ...
    var init = function (dataVariable) {
       ...
       CommanJS.get_metadata_options("TEL mastery standards", 1, 15, "selected_standard_cat");
        ...
        CommanJS.get_metadata_options("Learning outcomes", 1, 14, "selected_outcomes_cat");
        CommanJS.get_metadata_options("Learning standards", 1, 15, "selected_standard_cat");
        ...
        $('#selected_filter_search').on('change', '.metadata_selector', function() {
           selectedTable.search('').draw();
        });
        ...
        $(".remove-questions-from-section").on('click', function(){
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function () {
                        getRemoveQuestionFromSections(questions,quizID);    
                    },
                    cancel: function () {
                    }
                }
