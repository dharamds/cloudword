...
$('ul.acc-menu a').click(function() {
    localStorage.setItem("childUrl", $(this).attr('href'));
});

$('ul.dropdown-menu a').click(function() {
    logutUlr= $(this).attr('href');
    var logout = logutUlr.search("logout");
    if(logout !='-1'){
        localStorage.setItem("childUrl", null);
    }
});
    ...
    $.sidebarAccordion = function(element, options) {
        ...
        plugin.init = function() {
            ...
            var targetAnchor;
             childUrl = localStorage.getItem("childUrl");
            $.each ($('ul.acc-menu a'), function() {
                if( this.href == window.location || (childUrl!=null && this.href==childUrl)) {
                    targetAnchor = this;
                    return false;
                };
            });
